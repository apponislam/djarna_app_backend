...
const isDescendant = async (parentId: string, childId: string): Promise<boolean> => {
    ...
    const parent = await Category2Model.findById(parentId).lean();
...
const getMaxLevelInSubtree = async (categoryId: string, currentMax: number): Promise<number> => {
    const children = await Category2Model.find({ parentCategory: categoryId }).lean();
...
const updateDescendantLevels = async (parentId: string, levelOffset: number) => {
    const children = await Category2Model.find({ parentCategory: parentId });
    for (const child of children) {
        const newLevel = child.level + levelOffset;
        await Category2Model.findByIdAndUpdate(child._id, { level: newLevel });
...
const getBreadcrumbs = async (categoryId: string): Promise<any[]> => {
    const path: any[] = [];
    let current = await Category2Model.findById(categoryId).lean();
    while (current) {
        path.unshift({
            _id: current._id,
            name: current.name,
        ...
        if (current.parentCategory) {
            current = await Category2Model.findById(current.parentCategory).lean();
        } else {
            break;
        }
    }
...
const createCategory = async (payload: ICategory2) => {
    ...
    if (payload.parentCategory) {
        const parent = await Category2Model.findById(payload.parentCategory);
        if (!parent) {
    ...
    const isExist = await Category2Model.findOne({
    ...
    if (payload.homePosition !== null && payload.homePosition !== undefined) {
        const positionExists = await Category2Model.findOne({ homePosition: payload.homePosition });
        if (positionExists) {
    ...
    const result = await Category2Model.create(categoryData);
    return result;
};

const getCategoryById = async (id: string) => {
    const category = await Category2Model.findById(id).lean();
    if (!category) {
        throw new ApiError(httpStatus.NOT_FOUND, "Category not found!");
    }

    const subcategories = await Category2Model.find({ parentCategory: id }).lean();
...
const updateCategory = async (id: string, payload: Partial<ICategory2>) => {
    const category = await Category2Model.findById(id);
    if (!category) {
        throw new ApiError(httpStatus.NOT_FOUND, "Category not found!");
    }
    ...
    if (payload.parentCategory !== undefined && newParentId !== currentParentId) {
        if (newParentId) {
            ...
            const parent = await Category2Model.findById(newParentId);
            if (!parent) {
                throw new ApiError(httpStatus.NOT_FOUND, "Parent category not found!");
            }
    ...
    if (payload.homePosition !== null && payload.homePosition !== undefined) {
        const positionExists = await Category2Model.findOne({
    ...
    const result = await Category2Model.findByIdAndUpdate(id, updateData, { returnDocument: "after" });
...
const deleteCategory = async (id: string) => {
    const isExist = await Category2Model.findById(id);
    if (!isExist) {
        throw new ApiError(httpStatus.NOT_FOUND, "Category not found!");
    }

    // Check if it has subcategories before deleting
    const subcategories = await Category2Model.find({ parentCategory: id });
    if (subcategories.length > 0) {
        throw new ApiError(httpStatus.BAD_REQUEST, "Cannot delete category with subcategories!");
    }

    const result = await Category2Model.findByIdAndDelete(id);
    return result;
};
...
const getParentCategories = async (searchTerm?: string) => {
    ...
    const categories = await Category2Model.find(query).sort({ homePosition: 1, name: 1 }).lean();

    const result = await Promise.all(
        categories.map(async (category) => {
            const subcategoryCount = await Category2Model.countDocuments({ parentCategory: category._id });
            return {
                ...category,
                subcategoryCount,
            };
...
const getCategoryTree = async (query: any = {}) => {
    ...
    const allCategories = await Category2Model.find(filter).sort({ homePosition: 1, name: 1 }).lean();
